...
# 加载XML文件
def modify_xmlfile (file_path,env_list):
...
def process_zip_file(zip_file_path,output_folder,file_name,env_list):
        ...
        for item in original_zip.infolist():
            ...
            if item.filename == file_name:
                ...
                with original_zip.open(item) as versioninfo_path:
                    #修改versioninfo.xml文件
                    content = modify_xmlfile(versioninfo_path.read(),env_list)
            # 将文件内容写入更新后的 ZIP 文件
...
xml_file_name = 'META-INF/versioninfo.xml'
 # 读取配置文件内容
with open('versioninfo.config', 'r') as file:
    config_content = file.read()

# 将配置文件内容按行分割成列表
config_list = config_content.splitlines()

# 更新 env_list 变量的值为配置文件内容
...
for zip_file in zip_files:
    ...
    process_zip_file(zip_file,out_file,xml_file_name,env_list)
